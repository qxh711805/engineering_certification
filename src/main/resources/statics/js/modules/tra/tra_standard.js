$(function () {
    ...
    $("#jqGrid").jqGrid({
        url: baseURL + 'tra/standard/capability/list',
        datatype: "json",
        colModel: [
            {label: '标准能力记录流水号', name: 'id', index: "id", width: 40, key: true, hidden: true},
            // { label: '用户名', name: 'username', width: 75 },
            {label: '认证标准', name: 'standard', width: 70},
            {label: '版本', name: 'version', width: 70},
            {label: '发布单位', name: 'issuedBy', sortable: false, width: 70},
    ...
    methods: {
        ...
        del: function (ids) {
            confirm('确定要删除选中的记录？', function () {
                $.ajax({
                    type: "POST",
                    url: baseURL + "tra/standard/capability/delete",
        ...
        saveOrUpdate: function () {
            var url = vm.dataInfo.id == null ? "tra/standard/capability/save" : "tra/standard/capability/update";
            $.ajax({
                type: "POST",
                url: baseURL + url,
                contentType: "application/json",
        ...
        getInfo: function (id) {
            $.get(baseURL + "tra/standard/capability/info/" + id, function (r) {
                vm.dataInfo = r.data;
            });
        },
        excelImp: function (event) {
            var diaindx = layer.open({
                ...
                yes: function (index, layero) {
                    ...
                    $.get(baseURL + 'tra/excel/saveImport/standardCapability_info_in_session', function (r) {
                        if(r.code<0){
                ...
                cancel: function (index, layero) {
                    $.get(baseURL + 'tra/excel/removeSession/standardCapability_info_in_session', function (r) {
                    });
                },

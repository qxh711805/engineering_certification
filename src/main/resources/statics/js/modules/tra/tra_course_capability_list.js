$(function () {
    ...
    $("#jqGrid").jqGrid({
        url: baseURL + 'tra/course/capability/list',
        datatype: "json",
        colModel: [
            {label: '课程能力清单 流水号', name: 'id', index: "id", width: 80, key: true, hidden: true},
            {label: '课程编号', name: 'courseNumber', width: 80},
            {label: '能力', name: 'capability', width: 80},
            {label: '权重', name: 'weight', sortable: false, width: 80},
            {label: '课程名称', name: 'courseName', sortable: false, width: 80},
    ...
    methods: {
        ...
        del: function (ids) {
            confirm('确定要删除选中的记录？', function () {
                $.ajax({
                    type: "POST",
                    url: baseURL + "tra/course/capability/delete",
        ...
        saveOrUpdate: function () {
            var url = vm.dataInfo.id == null ? "tra/course/capability/save" : "tra/course/capability/update";
            $.ajax({
                type: "POST",
                url: baseURL + url,
                contentType: "application/json",
        ...
        getInfo: function (id) {
            $.get(baseURL + "tra/course/capability/info/" + id, function (r) {
                vm.dataInfo = r.data;
            });
        },
        excelImp: function (event) {
            var diaindx = layer.open({
                ...
                yes: function (index, layero) {
                    ...
                    $.get(baseURL + 'tra/excel/saveImport/courseCapability_info_in_session', function (r) {
                        if(r.code<0){
                ...
                cancel: function (index, layero) {
                    $.get(baseURL + 'tra/excel/removeSession/courseCapability_info_in_session', function (r) {
                    });
                },

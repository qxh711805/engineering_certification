$(function () {
    ...
    $("#jqGrid").jqGrid({
        url: baseURL + 'tra/major/capability/target/list',
        datatype: "json",
        colModel: [
            {label: '专业培养目标 流水号', name: 'id', index: "id", width: 80, key: true, hidden: true},
            {label: '编号', name: 'number', width: 80},
            {label: '标题', name: 'title', width: 80},
            {label: '描述', name: 'description', sortable: false, width: 80},
    ...
    methods: {
        ...
        del: function (ids) {
            confirm('确定要删除选中的记录？', function () {
                $.ajax({
                    type: "POST",
                    url: baseURL + "tra/major/capability/target/delete",
        ...
        saveOrUpdate: function () {
            var url = vm.dataInfo.id == null ? "tra/major/capability/target/save" : "tra/major/capability/target/update";
            $.ajax({
                type: "POST",
                url: baseURL + url,
                contentType: "application/json",
        ...
        getInfo: function (id) {
            $.get(baseURL + "tra/major/capability/target/info/" + id, function (r) {
                vm.dataInfo = r.data;
            });
        },
        excelImp: function (event) {
            var diaindx = layer.open({
                ...
                yes: function (index, layero) {
                    ...
                    $.get(baseURL + 'tra/excel/saveImport/majorCapabilityTarget_info_in_session', function (r) {
                        if(r.code<0){
                ...
                cancel: function (index, layero) {
                    $.get(baseURL + 'tra/excel/removeSession/majorCapabilityTarget_info_in_session', function (r) {
                    });
                },
